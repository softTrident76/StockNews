...
$q = "select symbol,last_price from sn_stocks where powr_number < 1.5 order by symbol asc";
...
$unchanged = 0;
foreach ($r as $row) {
    $q2 = "select h.day_close, h.date from sn_historical h where symbol='".$row->symbol."' and h.date = (select max(date) from sn_historical where symbol='".$row->symbol."' and powr_number > 1.5)";
...
echo "<tr><td><strong>Total D-rated stocks</strong></td><td>$count</td></tr>"; //$count
echo "<tr><td><strong>Avg. % return since downgrade to D</strong></td><td>$avg%</td></tr>";
echo "<tr><td><strong>Avg. # days since downgraded to D</strong></td><td>$avg_days</td></tr>";
